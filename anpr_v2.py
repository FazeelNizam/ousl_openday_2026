...
2. For smooth, jitter-free servo control, use the pigpio daemon for
   hardware-timed PWM instead of software PWM:
...
4. Python packages (a virtualenv with --system-site-packages is recommended
   so it can see the apt-installed picamera2 and RPi.GPIO):
       ...
       pip install ultralytics easyocr RPLCD gpiozero pigpio
...
class GateController:
    ...
    def __init__(self, pin, min_pulse_width, max_pulse_width):
        try:
            # pigpio gives hardware-timed pulses -> much less servo jitter
            # than the Pi's default software PWM pin factory.
            factory = PiGPIOFactory()
        except Exception as e:
            print(f"[Gate] Could not connect to pigpiod ({e}); falling back to "
                  f"software PWM. Run `sudo systemctl start pigpiod` for smoother motion.")
            factory = None
