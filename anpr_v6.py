...
4. Python packages (a virtualenv with --system-site-packages is recommended
   ...
   IMPORTANT - install CPU-only PyTorch FIRST, before ultralytics/easyocr:
   as of PyTorch 2.11 (April 2026), a plain `pip install torch` on aarch64
   Linux (your Pi's architecture) defaults to a CUDA-enabled build and
   pulls several hundred MB of unusable `nvidia-*` packages
   (nvidia-cublas-cu12, nvidia-cudnn-cu12, etc.) even though the Pi has no
   NVIDIA GPU. Get the CPU-only build from PyTorch's own index instead:
       pip install torch torchvision --index-url https://download.pytorch.org/whl/cpu
   Then install the rest - pip will see torch is already satisfied and
   ...
   Verify no CUDA/nvidia packages snuck in:
       pip list | grep -i nvidia
   This should print nothing. If it does, remove them:
       pip uninstall -y $(pip list | grep -i '^nvidia-' | awk '{print $1}')
   and re-check that `python3 -c "import torch; print(torch.version.cuda)"`
   prints None.
...
# instead of reacting to (or printing) every single unstable guess.
PLATE_CONFIRM_COUNT = 2
    ...
    def _process_frame(self, frame):
        try:
            ...
            for i in range(len(detections)):
                ...
                # read on PLATE_CONFIRM_COUNT consecutive detections.
