...
# --- Quiet down noisy third-party warnings/logs BEFORE importing them ---
warnings.filterwarnings("ignore")                         # PyTorch/gpiozero UserWarnings etc.
os.environ["LIBCAMERA_LOG_LEVELS"] = "*:ERROR"             # silence libcamera's own C++ INFO logs
os.environ["YOLO_VERBOSE"] = "False"                       # silence ultralytics' load/inference banners
...
# OCR often splits one plate into several text fragments (e.g. 'NKTI' +
# '2282'). Fragments below OCR_MIN_FRAGMENT_CONF are dropped as pure noise;
# the rest are joined left-to-right and judged as a WHOLE by their average
# confidence against OCR_THRESH - NOT by requiring every single fragment to
# individually clear a high bar, which in practice almost never happens for
# all fragments at once and silently starves out real plates.
OCR_MIN_FRAGMENT_CONF = 0.15
OCR_THRESH = 0.60

# A plate is only acted on after being read as the SAME normalized text on
# this many consecutive detections, to smooth over frame-to-frame OCR noise.
# On a Pi 4B, EasyOCR takes ~1-3s per call, so only a couple of frames get
# analyzed while a vehicle is in view - requiring 2+ identical reads out of
# that handful is often too strict and just times out waiting. Set to 1 to
# act on the first read that clears OCR_THRESH (relies on OCR_THRESH alone
# for reliability); raise to 2+ only if you're seeing false triggers.
PLATE_CONFIRM_COUNT = 1

# If a candidate plate doesn't get its confirming re-read within this many
# seconds (car moved out of frame, lighting changed, etc.), give up waiting
# on it and start fresh with whatever comes next, instead of holding onto a
# stale candidate indefinitely.
...
class GateController:
    ...
    def __init__(self, pin, min_pulse_width, max_pulse_width):
        ...
        try:
            ...
            # The pigpio module prints a big "Can't connect to pigpio..."
            # banner straight to stdout/stderr on failure - swallow that
            # noise here and report the failure ourselves, once, cleanly.
...
class ANPRApp:
    def __init__(self):
        ...
        # Debounce state: require the same normalized plate on
        # PLATE_CONFIRM_COUNT consecutive detections before acting on it.
        self._candidate_plate = None
    ...
    def _process_frame(self, frame):
        try:
            ...
            for i in range(len(detections)):
                ...
                print(f"[YOLO] Plate detected (confidence={conf:.2f})")
                ...
                # A plate is often split into several text boxes (e.g.
                # 'NKTI' + '2282'). Order them left-to-right so they
                # concatenate in reading order, drop only near-garbage
                # fragments, and judge the combined string by its AVERAGE
                # confidence rather than demanding every single fragment
                # individually clear a high bar (which rarely happens for
                # all fragments on the same frame).
                fragments = [
                    (text, prob) for (bbox, text, prob) in ocr_results
                    if text and prob >= OCR_MIN_FRAGMENT_CONF
                ]

                if not fragments:
                    print("[OCR] No usable text in plate crop")
                    continue

                # sort left-to-right using each fragment's bounding box
                ordered = sorted(
                    [(bbox, text, prob) for (bbox, text, prob) in ocr_results
                     if text and prob >= OCR_MIN_FRAGMENT_CONF],
                    key=lambda item: min(pt[0] for pt in item[0])
                )
                combined_text = "".join(text for (_, text, _) in ordered)
                avg_conf = sum(prob for (_, _, prob) in ordered) / len(ordered)

                print(f"[OCR] Combined reading: '{combined_text}' "
                      f"(avg confidence={avg_conf:.2f}, {len(ordered)} fragment(s))")

                if avg_conf < OCR_THRESH or len(combined_text) < 6:
                    continue

                filtered_plate = filter_reading(combined_text)
                if not filtered_plate:
                    continue

                # Debounce: only act once the same normalized plate has been
                # read on PLATE_CONFIRM_COUNT consecutive detections, and
                # give up on a stale candidate that never gets its
                # confirming re-read within CANDIDATE_TIMEOUT_SECONDS.
                now = time.time()
                candidate_expired = (
                    self._candidate_plate is not None
                    and (now - self._candidate_first_seen) > CANDIDATE_TIMEOUT_SECONDS
                )
                if candidate_expired:
                    print(f"[Plate] Candidate '{self._candidate_plate}' expired "
                          f"(no confirmation within {CANDIDATE_TIMEOUT_SECONDS:.0f}s) - resetting")
                    self._candidate_plate = None
                    self._candidate_count = 0

                if filtered_plate == self._candidate_plate:
                    self._candidate_count += 1
                else:
                    self._candidate_plate = filtered_plate
                    self._candidate_count = 1
                    self._candidate_first_seen = now

                print(f"[Plate] Candidate '{filtered_plate}' "
                      f"({self._candidate_count}/{PLATE_CONFIRM_COUNT})")

                if self._candidate_count >= PLATE_CONFIRM_COUNT:
